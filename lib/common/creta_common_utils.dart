...
class CretaCommonUtils {
  ...
  static String getFontName(String font) {
    ...
    if (font == "NotoSansJP") {
      return CretaLang['fontNoto_Sans_JP']!;
    }
    if (font == "NotoSansKR") {
  ...
  static String getFontFamily(String font) {
    if (font == CretaLang['fontNoto_Sans_JP']!) {
      return "NotoSansJP";
    }
...
// 텍스트가 차이하는 실제 화면상의 사이즈를 구해준다.
  ...
  static Widget underConstruction(
      {required double width, double? height, EdgeInsetsGeometry? padding}) {
    return
        // Positioned.fill(
        //   child:
        Container(
      ...
      child: Center(
        child: Image(
          ...
          height: height != null ? min(width, height) - 50 : width - 50,
        ),
      ),
      //),
    );
